...
unspent_tx_out_t *find_unspent_output(llist_t *all_unspent, tx_in_t *in)
{
	...
	for (i = 0, size = llist_size(all_unspent); i < size; i++)
	{
		...
		if (
			!memcmp(tmp_unspent->out.hash, in->tx_out_hash,
					sizeof(tmp_unspent->out.hash)) &&
			!memcmp(tmp_unspent->block_hash, in->block_hash,
					sizeof(tmp_unspent->block_hash))
			)
			return (tmp_unspent);
	}
							...
							llist_t *all_unspent)
{
	...
	for (i = 0, size = llist_size(transaction->inputs); i < size; i++)
	{
		...
		if (!(tmp_unspent = find_unspent_output(all_unspent, tmp_input)) ||
			!ec_verify(ec_from_pub(tmp_unspent->out.pub), transaction->id,
			SHA256_DIGEST_LENGTH, &tmp_input->sig)
		)
			return (0);
